...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.vendedor) {
        _push(`<div class="border-b border-jade/30 bg-jade/10 px-4 py-2 text-center text-14 text-jade print:hidden" role="status"> Você está no link de <span class="font-bold">${ssrInterpolate(__props.vendedor.nome)}</span>. </div>`);
      } else {
        _push(`<!---->`);
      }
      if (rodada.value) {
        ...
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`Área do vendedor`);
            } else {
              return [
                createTextVNode("Área do vendedor")
              ];
            }
          }),
          _: 1
